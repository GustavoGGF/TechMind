...
namespace Utils
{
    public class Loader
    {
        ...
        {
            // Verifica se a barra retangular foi inicializada e está presente no formulário
            if (this.barraRetangular != null && form.Controls.Contains(this.barraRetangular))
            {
                // Remove a barra retangular do formulário
                form.Controls.Remove(this.barraRetangular);
                this.barraRetangular.Dispose(); // Libera recursos do painel
                this.barraRetangular = null; // Define como null para indicar que foi removido
            }
        }
    }
